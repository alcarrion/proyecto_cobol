      *================================================================*
      * PROGRAMA : DBIOBORM.cob                                        *
      * FUNCION  : Capa de Acceso a Datos - Hipotecas v2.0             *
      * NIVEL    : 1000 TCS ECUADOR                                     *
      * SCHEMA   : control_secuencias (PRODUCTO/ULTIMO_VALOR)          *
      *            hipotecas v2.0 (MONTO_PRESTAMO, SALDO_DEUDA,        *
      *            TASA_ANUAL, CUENTA_DEBITO, FECHA_VENCIMIENTO,       *
      *            ESTADO_PRESTAMO)                                     *
      * ACCIONES:                                                       *
      *   S = UPDATE+SELECT control_secuencias                         *
      *   A = INSERT hipotecas nuevo schema                             *
      *   C = SELECT por ID_HIPOTECA                                    *
      *   M = UPDATE SALDO_DEUDA, ESTADO_PRESTAMO, MESES_MORA          *
      *       Si ESTADO='PAGADO': UPDATE clientes SET TIENE_HIPOTECA=0  *
      *================================================================*
      ...
      **********************************************************************
      * STMT-0: UPDATE control_secuencias incrementar
       01 SQL-STMT-0.
           ...
           05 SQL-STMT   PIC X(88) VALUE 'UPDATE control_secuencias SET
      -    'ULTIMO_VALOR = ULTIMO_VALOR + 1 WHERE PRODUCTO = ''HIPOTECA'
      -    ''''.
      **********************************************************************
      * STMT-1: SELECT ULTIMO_VALOR FROM control_secuencias
       01 SQL-STMT-1.
           05 SQL-IPTR   POINTER VALUE NULL.
           05 SQL-PREP   PIC X VALUE 'N'.
           05 SQL-OPT    PIC X VALUE SPACE.
           ...
           05 SQL-STMT   PIC X(67) VALUE 'SELECT ULTIMO_VALOR FROM contr
      -    'ol_secuencias WHERE PRODUCTO = ''HIPOTECA'''.
      **********************************************************************
      * STMT-2: INSERT hipotecas v2.0 (8 params)
       01 SQL-STMT-2.
           05 SQL-IPTR   POINTER VALUE NULL.
           05 SQL-PREP   PIC X VALUE 'N'.
           05 SQL-OPT    PIC X VALUE SPACE.
      ...
      -    'IMIENTO) VALUES (?,?,?,?,?,?,?,?,?,?)'.
      **********************************************************************
      * STMT-3: SELECT hipoteca por ID_HIPOTECA
       01 SQL-STMT-3.
           05 SQL-IPTR   POINTER VALUE NULL.
           05 SQL-PREP   PIC X VALUE 'N'.
           05 SQL-OPT    PIC X VALUE SPACE.
           05 SQL-PARMS  PIC S9(4) COMP-5 VALUE 1.
           05 SQL-STMLEN PIC S9(4) COMP-5 VALUE 190.
           05 SQL-STMT   PIC X(190) VALUE 'SELECT ID_CLIENTE,CUENTA_DEBI
      -    'TO,MONTO_PRESTAMO,SALDO_DEUDA,TASA_ANUAL,CUOTA_MENSUAL,MESES
      -    '_MORA,ESTADO_PRESTAMO,FECHA_VENCIMIENTO FROM hipotecas WHERE
      -    'ID_HIPOTECA = ?'.
      **********************************************************************
      * STMT-4: UPDATE hipoteca SALDO_DEUDA, ESTADO_PRESTAMO, MESES_MORA
       01 SQL-STMT-4.
           05 SQL-IPTR   POINTER VALUE NULL.
           05 SQL-PREP   PIC X VALUE 'N'.
           05 SQL-OPT    PIC X VALUE SPACE.
           ...
           05 SQL-STMT   PIC X(105) VALUE 'UPDATE hipotecas SET SALDO_DE
      -    'UDA = ?,ESTADO_PRESTAMO = ?,MESES_MORA = ? WHERE ID_HIPOTECA
      -    ' = ?'.
      **********************************************************************
      * STMT-5: UPDATE clientes SET TIENE_HIPOTECA=0 cuando PAGADO
       01 SQL-STMT-5.
           05 SQL-IPTR   POINTER VALUE NULL.
           05 SQL-PREP   PIC X VALUE 'N'.
           05 SQL-OPT    PIC X VALUE SPACE.
           ...
           05 SQL-STMT   PIC X(58) VALUE 'UPDATE clientes SET TIENE_HIPO
      -    'TECA = 0 WHERE ID_CLIENTE = ?'.
      **********************************************************************
      * STMT-6: UPDATE clientes SET TIENE_HIPOTECA=1 cuando alta
       01 SQL-STMT-6.
           05 SQL-IPTR   POINTER VALUE NULL.
           05 SQL-PREP   PIC X VALUE 'N'.
           05 SQL-OPT    PIC X VALUE SPACE.
      ...
      *******          PRECOMPILER-GENERATED VARIABLES               *******
       01 SQLV-GEN-VARS.
           ...
           05 SQL-VAR-0005  PIC S9(9)V9(2) COMP-3.
      ...
      *    Variables de trabajo locales
       01  WS-ID-HIPOTECA         PIC 9(09).
       01  WS-ID-CLIENTE          PIC 9(09).
       01  WS-CUENTA-DEBITO       PIC 9(09).
       01  WS-MONTO-PRESTAMO      PIC S9(15)V99.
       01  WS-SALDO-DEUDA         PIC S9(15)V99.
       01  WS-TASA-ANUAL          PIC S9(07)V9999.
       01  WS-CUOTA-MENSUAL       PIC S9(15)V99.
       ...
       01  LK-DATOS-TRANSACCION.
           ...
           05 LK-ID-CUENTA             PIC 9(09).
           05 LK-IMPORTE               PIC S9(15)V99 COMP-3.
           05 LK-COD-RETORNO           PIC X(04).
              88 LK-EXITO              VALUE '00  '.
           05 LK-MENSAJE               PIC X(60).
           05 LK-USUARIO-ID            PIC X(08).
           05 LK-TERMINAL-ID           PIC X(04).
           05 LK-FECHA-PROCESO         PIC X(10).

       01  BORM-REGISTRO.
           05 BORM-ID-HIPOTECA         PIC 9(09).
           05 BORM-ID-CLIENTE          PIC 9(09).
           05 BORM-CUENTA-DEBITO       PIC 9(09).
           05 BORM-MONTO-PRESTAMO      PIC S9(15)V99.
           05 BORM-SALDO-DEUDA         PIC S9(15)V99.
           05 BORM-TASA-ANUAL          PIC S9(07)V9999.
           05 BORM-CUOTA-MENSUAL       PIC S9(15)V99.
           05 BORM-MESES-MORA          PIC 9(03).
           05 BORM-ESTADO-PRESTAMO     PIC X(15).
           05 BORM-FECHA-VENCIMIENTO   PIC X(10).
      ...
      *        END-EXEC
       ...
       1000-INSERTAR-HIPOTECA.
           ...
           MOVE BORM-CUENTA-DEBITO   TO WS-CUENTA-DEBITO.
           MOVE BORM-MONTO-PRESTAMO  TO WS-MONTO-PRESTAMO.
           MOVE BORM-SALDO-DEUDA     TO WS-SALDO-DEUDA.
           MOVE BORM-TASA-ANUAL      TO WS-TASA-ANUAL.
           MOVE BORM-CUOTA-MENSUAL   TO WS-CUOTA-MENSUAL.
           MOVE BORM-MESES-MORA      TO WS-MESES-MORA.
           MOVE BORM-ESTADO-PRESTAMO TO WS-ESTADO-PRESTAMO.
           MOVE BORM-FECHA-VENCIMIENTO TO WS-FECHA-VENCIMIENTO.

      *    EXEC SQL
      *        INSERT INTO hipotecas (
      *            ID_HIPOTECA, ID_CLIENTE, CUENTA_DEBITO,
      *            MONTO_PRESTAMO, SALDO_DEUDA, TASA_ANUAL,
      *            CUOTA_MENSUAL, MESES_MORA, ESTADO_PRESTAMO,
      *            FECHA_VENCIMIENTO
      *        ) VALUES (?,?,?,?,?,?,?,?,?,?)
      *    END-EXEC.
           ...
           IF LK-COD-RETORNO = '00  '
      *        UPDATE clientes SET TIENE_HIPOTECA = 1
      ...
      *    END-EXEC.
           ...
           IF LK-COD-RETORNO = '00  '
               ...
               MOVE WS-MONTO-PRESTAMO    TO BORM-MONTO-PRESTAMO
               MOVE WS-SALDO-DEUDA       TO BORM-SALDO-DEUDA
               MOVE WS-TASA-ANUAL        TO BORM-TASA-ANUAL
               MOVE WS-CUOTA-MENSUAL     TO BORM-CUOTA-MENSUAL
               MOVE WS-MESES-MORA        TO BORM-MESES-MORA
               MOVE WS-ESTADO-PRESTAMO   TO BORM-ESTADO-PRESTAMO
               MOVE WS-FECHA-VENCIMIENTO TO BORM-FECHA-VENCIMIENTO
      ...
      *            UPDATE clientes SET TIENE_HIPOTECA = 0
      ...
      *  DBIOBORM: Capa DBIO hipotecas - schema v2.0
      *  Acciones: S A C M
      **********************************************************************
