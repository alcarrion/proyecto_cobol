      ******************************************************************
      * DBIOBORM.SQB - CAPA DE ACCESO A DATOS - HIPOTECAS
      * v4.0 - Validaciones de entrada + mensajes consistentes
      * TABLA    : hipotecas, control_secuencias, clientes
      * ACCIONES : S=Secuencia  A=Alta  C=Consulta  M=Modificar
      * NOTA     : BAT000 usa CUENTA_DEBITO para el debito mensual.
      ******************************************************************
      ...
      *        Valida datos antes del INSERT para evitar errores de BD.
      ******************************************************************
       1000-INSERTAR-HIPOTECA.
           PERFORM 1050-VALIDAR-DATOS-ALTA.
      ...
      ******************************************************************
      * 1050 - VALIDACIONES PREVIAS AL INSERT
      ******************************************************************
       1050-VALIDAR-DATOS-ALTA.
           EVALUATE TRUE
               WHEN LK-BORM-ID-HIPOTECA = ZERO
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "ID de hipoteca invalido" TO LK-MENSAJE
               WHEN LK-BORM-ID-CLIENTE = ZERO
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "ID de cliente invalido" TO LK-MENSAJE
               WHEN LK-BORM-CUENTA-DEBITO = ZERO
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "Cuenta debito invalida" TO LK-MENSAJE
               WHEN LK-BORM-MONTO-PRESTAMO <= 0
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "Monto prestamo debe ser mayor a cero"
                       TO LK-MENSAJE
               WHEN LK-BORM-CUOTA-MENSUAL <= 0
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "Cuota mensual debe ser mayor a cero"
                       TO LK-MENSAJE
               WHEN LK-BORM-TASA-ANUAL <= 0
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "Tasa anual debe ser mayor a cero"
                       TO LK-MENSAJE
               WHEN LK-BORM-ESTADO-PRESTAMO = SPACES
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "Estado prestamo requerido" TO LK-MENSAJE
               WHEN LK-BORM-FECHA-VENCIMIENTO = SPACES
                   MOVE 'E001' TO LK-COD-RETORNO
                   MOVE "Fecha vencimiento requerida" TO LK-MENSAJE
               WHEN OTHER
                   MOVE '00  ' TO LK-COD-RETORNO
           END-EVALUATE.
      ...
      ******************************************************************
      * 3000 - ACTUALIZAR HIPOTECA (pago manual y debito batch)
      *        Valida que la hipoteca exista y no este ya pagada.
      ******************************************************************
       3000-ACTUALIZAR-HIPOTECA.
           ...
           IF LK-BORM-SALDO-DEUDA < 0
               MOVE 'E001' TO LK-COD-RETORNO
               MOVE "Saldo deuda no puede ser negativo" TO LK-MENSAJE
               EXIT PARAGRAPH
           END-IF.

           IF LK-BORM-ESTADO-PRESTAMO = SPACES
               MOVE 'E001' TO LK-COD-RETORNO
               MOVE "Estado prestamo requerido" TO LK-MENSAJE
               EXIT PARAGRAPH
           END-IF.
